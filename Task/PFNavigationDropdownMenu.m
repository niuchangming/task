...
- (instancetype)initWithFrame:(CGRect)frame
    ...
    if (self) {
        ...
        self.tableView.pfTableDelegate = self;
    }
    return self;
}

-(void) didPFTableSelectedAtIndex:(NSInteger)index{
    [self setMenuTitleText:[_items[index] name]];
    [self hideMenu];
    self.isShown = NO;
    [self layoutSubviews];
    
    if(delegate != nil && [delegate respondsToSelector:@selector(didDropdownMenuSelected:)]){
        [delegate didDropdownMenuSelected:index];
    }
}
